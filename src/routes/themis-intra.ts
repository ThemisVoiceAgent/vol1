...
import { startThemisCallStatusAutoPoll } from "../themis-intra/callStatusPoll.js";
  ...
  void (async () => {
    try {
      ...
      for (const client of clients) {
        ...
        // 5331 fix 2026-09-18: body moved verbatim to themis-intra/callStatusPoll.ts so the retry
        // path (processDueThemisRetries) shares the identical status write-back wiring.
        startThemisCallStatusAutoPoll({
          callId,
          twilioCallSid: result.twilio_call_sid,
          phone,
          debtAmountRaw: client.claim_remain,
        });
      }
